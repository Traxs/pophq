...
Officers schedule events (Foundry, SvS, King of Icefield/KOI, FDT, Canyon, Tundra League, other) with a start time. Answers close before the start according to the event: **Foundry three days before; SvS and KOI three hours before**. Other defaults remain configurable, and an officer can choose a one- or three-hour cutoff or a one-, two- or three-day cutoff per event. Day-based deadlines fall at the end of that day in the officer's time zone. Officers can edit an event later; moving the start moves the deadline with it.
...
The officer attendance analytics are person-level. Accounts linked to the same login are shown once under the earliest-linked main account, with its sub accounts named beneath it. Presence on any linked account wins for that event; otherwise an explicit absence counts. Missing, excused and unknown records are ignored rather than invented as attendance or absence. The underlying event records remain attached to their original game accounts.
...
When creating a non-Foundry event, the officer chooses its signup format: a simple **Joining / Not at all** RSVP, or **Full time / First half / Last half / Not at all** availability. SvS, KOI and FDT default to availability. KOI is tracked separately from SvS because it is the monthly state-internal event rather than a cross-state event.
