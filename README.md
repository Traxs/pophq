...
Tables carry small six-month trailing graphs: a line for Foundry strength, and a line for attendance on a fixed 0–100% scale with a faint halfway mark (green when more than half the commitments were kept, red below). A month with nothing recorded is a gap, never a zero, and a single reading shows as a dot rather than a fake trend.
