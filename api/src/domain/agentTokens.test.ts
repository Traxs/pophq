...
describe("agent tokens", () => {
  it("issues a one-time 256-bit secret and stores only its hash", () => {
    const issued = issueAgentToken({ name: "Hermes", scopes: ["results:read", "results:write"], expiresInDays: 30 }, "officer", new Date("2026-09-20T00:00:00Z"));
  ...
  it("limits scope and lifetime", () => {
    ...
    expect(() => issueAgentToken({ name: "Hermes", scopes: ["results:read"], expiresInDays: 91 }, "officer", new Date())).toThrow();
  });
