...
- **2026-09-21, the event taxonomy is Foundry, SvS, FDT, Canyon and Tundra League:** the Bear hunt runs every other day, needs no sign-up, and becomes a scores fixture instead (its type is archived, so past bear events still resolve). SvS and FDT ask **Full time / First half / Last half**, which is the parts mechanism, not a new answer vocabulary. Canyon and Tundra League are an RSVP: one button to join, the same button to drop out, and no "maybe" — an officer cannot plan on a maybe. Lead times for the three new types start at a day and are still to be settled per type.
