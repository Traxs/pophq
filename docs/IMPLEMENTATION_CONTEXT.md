...
- Person-level attendance analytics group every login's linked accounts under its earliest-linked main account. Presence on any linked account wins over absence for the same event, and a confirmed positive player score is also presence evidence. Foundry L1/L2 legacy events on the same UTC day are one opportunity. An occurrence enters the alliance denominator only after explicit attendance review, so a partial scoreboard cannot silently mark every omitted member absent; the underlying answers, attendance, and scores remain account-owned.
